...
class RenderEffectImageProcessor : ImageProcessor {
    ...
    private fun applyEffect(it: Params, renderEffect: RenderEffect, input: Bitmap): Bitmap {
        it.renderNode.setRenderEffect(renderEffect)
        val renderCanvas = it.renderNode.beginRecording()
        renderCanvas.drawBitmap(input, 0f, 0f, null)
    ...
    override fun rotateHue(radian: Float, outputIndex: Int): Bitmap {
        params?.let {
            ...
            return applyEffect(it, colorFilterEffect, Bitmap.createBitmap(1,1,Bitmap.Config.ARGB_8888))
        }
        throw RuntimeException("Not configured!")
    }

    override fun blur(radius: Float, outputIndex: Int, inputImage: Bitmap): Bitmap {
        params?.let {
            val blurRenderEffect = RenderEffect.createBlurEffect(
                radius, radius,
                Shader.TileMode.MIRROR
            )
            return applyEffect(it, blurRenderEffect, inputImage)
        }
        throw RuntimeException("Not configured!")
    }
